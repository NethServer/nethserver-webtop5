...
<?php
/**
	This script extracts contacts from PST and add them to WebTop5.
**/
define("DEBUG",false);
...
echo "\nUser: $user\nFile: $file2import\nFolder: $foldername\n\n";
$confirm=readline("Confirm? [Y]es/[N]o/[T]est: ");
	...
	foreach($vcards as $data) {
		...
		if ($TEL['voice'])
                ...
                if (isset($ADR['work']['street'])) {
                        $arrayContact["work_address"] = truncateString($ADR['work']['street'], 100);
                }
                if (isset($ADR['work']['city'])) {
                        $arrayContact["work_city"] = truncateString($ADR['work']['city'], 30);
                }
                if (isset($ADR['work']['prov'])) {
                        $arrayContact["work_state"] = truncateString($ADR['work']['prov'], 30);
                }
                if (isset($ADR['work']['code'])) {
                        $arrayContact["work_postalcode"] = truncateString($ADR['work']['code'], 20);
                }
                if (isset($ADR['work']['country'])) {
                        $arrayContact["work_country"] =truncateString($ADR['work']['country'], 30);
                }
                if (isset($TEL['work'])) {
                        $arrayContact["work_telephone"] = truncateString($TEL['work'], 50);
                }
                if (isset($TEL['fax'])) {
                        $arrayContact["work_fax"] = truncateString($TEL['fax'], 50);
                }
                if (isset($EMAIL['work'])) {
                        $arrayContact["work_email"] = truncateString($EMAIL['work'], 80);
                }
                if (isset($ADR['home']['street'])) {
                        $arrayContact["home_address"] = truncateString($ADR['home']['street'], 100);
                }
                if (isset($ADR['home']['city'])) {
                        $arrayContact["home_city"] = truncateString($ADR['home']['city'], 30);
                }
                if (isset($ADR['home']['prov'])) {
                        $arrayContact["home_state"] = truncateString($ADR['home']['prov'], 30);
                }
                if (isset($ADR['home']['code'])) {
                        $arrayContact["home_postalcode"] = truncateString($ADR['home']['code'], 20);
                }
                if (isset($ADR['home']['country'])) {
                        $arrayContact["home_country"] = truncateString($ADR['home']['country'], 30);
                }
                if (isset($TEL['home'])) {
                        $arrayContact["home_telephone"] = truncateString($TEL['home'], 50);
                }
                if (isset($TEL['cell'])) {
                        $arrayContact["work_mobile"] = truncateString($TEL['cell'], 50);
                        $arrayContact["home_mobile"] = truncateString($TEL['cell'], 50);
                }
                if (isset($EMAIL['home'])) {
                        $arrayContact["home_email"] = truncateString($EMAIL['home'], 80);
                }
		...
		if (!$dryrun) {
	                $result = pg_insert($webtop_db, 'contacts.contacts', $arrayContact);
	                if ($result == FALSE)
	                        echo "Error creating $FirstN $SeconN $ORG di $user su $foldername\n";
		}
                echo "DONE\n";
		$num_contacts++;
